...
def main():
    ...
    model = BiLSTMBERTClassifier().to(device)
    ...
    metrics = evaluate_predictions(
        y_test,
        test_preds,
        name="BiLSTM+BERT Test"
    )

    print(metrics)

    # ---- SAVE METRICS ----
    with open(f"{RESULTS_DIR}/bilstm_bert_metrics.json", "w") as f:
        json.dump(metrics, f, indent=4)

    print("Saved metrics to artifacts/results/bilstm_bert_metrics.json")
